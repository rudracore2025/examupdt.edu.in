...
import { Plus, Edit, Trash2, Filter, X, Search, BookOpen } from 'lucide-react';
...
export function AdminNotes() {
  ...
  const fetchNotes = async () => {
    try {
      setIsLoading(true);
      const data = await adminNotesApi.getAll();
      setNotes(data);
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await adminNotesApi.delete(deleteId);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await Promise.all(selectedIds.map((id) => adminNotesApi.delete(id)));
      setNotes(notes.filter((n) => !selectedIds.includes(n.id)));
      setSelectedIds([]);
      toast.success(`${selectedIds.length} notes deleted successfully`);
    } catch (error) {
  ...
  const hasActiveFilters =
    filters.search || filters.subject !== 'all' || filters.fileType !== 'all';
    ...
    <AdminLayout>
      <div className="space-y-4 sm:space-y-6 w-full max-w-7xl mx-auto">
        <div className="flex flex-col gap-4">
          <div className="space-y-1">
            <h1 className="text-[#0A0A0A]">Notes</h1>
        ...
        {showFilters && (
          <div className="bg-white rounded-lg p-4 sm:p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              <div>
                <label className="text-[#0A0A0A]/60 mb-1.5 block">File Type</label>
            ...
            <table className="w-full">
              <thead className="bg-[#F5F5F5] border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-[#0A0A0A]/60 uppercase">File Type</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredNotes.length > 0 ? (
                  filteredNotes.map((note) => (
                    <tr key={note.id} className="hover:bg-[#F5F5F5] transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <p className="max-w-xs truncate">{note.title}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-[#0A0A0A]/60">{note.fileType}</td>
                      <td className="px-4 py-3 text-[#0A0A0A]/60">{formatDate(note.uploadDate)}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <Button asChild variant="ghost" size="sm">
                            <Link to={`/admin/notes/edit/${note.id}`}>
                  ...
                  <tr>
                    <td colSpan={6}>
                      <EmptyState
                        ...
                        description={hasActiveFilters ? 'Try adjusting your filters' : 'Add your first note to get started'}
                        actionLabel={!hasActiveFilters ? 'Add Note' : undefined}
                        onAction={!hasActiveFilters ? () => navigate('/admin/notes/create') : undefined}
                      />
                    </td>
                  </tr>
                )}
                ...
                <div key={note.id} className="p-4">
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[#0A0A0A] mb-2 line-clamp-2">{note.title}</h3>
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <Badge variant="secondary" className="bg-blue-500/10 text-blue-600">
                          {note.subject}
                        </Badge>
                        <span className="text-[#0A0A0A]/60">{note.fileType}</span>
                      </div>
                      <p className="text-[#0A0A0A]/60 mb-3">{formatDate(note.uploadDate)}</p>
                    </div>
                  </div>
